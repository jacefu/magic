...
- **中性深灰基底**：`#141414` 中性纯深灰替代蓝紫色调，与 Magic 平台整体观感对齐；不依赖色相营造层次，靠**亮度梯度**（最深栏 → 列表栏 → 内容区）拉开纵深
...
| 用途 | CSS 变量 | 值 | 说明 |
|------|---------|-----|------|
| 最底层（工作区栏） | `--bg-deepest` | `#141414` | 最深一档，纯中性深灰 |
| 侧边栏 / 右侧面板 | `--bg-glass` | `rgba(26,26,26,0.95)` | 毛玻璃底色，配合 `backdrop-filter: blur(20px)` |
| 聊天区主体 | `--bg-primary` | `#1f1f1f` | 主内容区，比侧栏亮一档 |
| 输入框 / 卡片 | `--bg-surface` | `rgba(255,255,255,0.05)` | 极微白，区分层次 |
| Hover 状态 | `--bg-hover` | `rgba(255,255,255,0.04)` | 悬浮微亮 |
| Active / 选中状态 | `--bg-active` | 见 2.3 渐变选中 | 渐变半透明 |
| 用户面板 | `--bg-panel` | `rgba(26,26,26,0.8)` | 底部面板，配合 blur |
...
&::after {
  ...
  inset: 0;
  border-radius: 14px;
  background: #1F1F1F; /* 内部填充，露出 2px 渐变边框 */
}
```
...
| 用途 | 暗色值 | 浅色值 | 说明 |
|------|--------|--------|------|
| 工作区栏 | `#141414` | `#F0EFF4` | 暗色：纯中性深灰；浅色：薰衣草灰 |
| 侧边栏 | `rgba(26,26,26,0.95)` | `rgba(248,247,252,0.85)` | 暗色：深灰 + blur；浅色：暖白紫 + blur |
| 聊天区 | `#1f1f1f` | `#FFFFFF` | 暗色：比侧栏亮一档；浅色：纯白 |
| 成员面板 | `rgba(26,26,26,0.85)` | `rgba(248,247,252,0.7)` | 同侧边栏稍透 |
| 输入框/卡片 | `rgba(255,255,255,0.05)` | `rgba(0,0,0,0.02)` | 极微对比 |
| Hover | `rgba(255,255,255,0.04)` | `rgba(0,0,0,0.025)` | |
| 选中态 | 渐变 `(108,92,231,0.14)→(0,180,216,0.08)` | 渐变 `(108,92,231,0.08)→(0,180,216,0.05)` | 浅色更淡 |
| 用户面板 | `rgba(26,26,26,0.8)` | `rgba(240,239,244,0.6)` | |
...
| 工作区选中图标内部填充 | `#1F1F1F` | `#5B50D6`（品牌紫，浅色下内部为实色） |
...
[data-theme="dark"] {
  ...
  --bg-active: linear-gradient(135deg, rgba(108,92,231,0.14), rgba(0,180,216,0.08));
  --bg-base: #141414;
